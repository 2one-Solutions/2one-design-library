#!/usr/bin/env node
/*
  2one DLS — MCP server (Phase 0, stdio).

  Exposes the design system as MCP tools + resources so an MCP client (Claude
  Desktop / Claude Code) builds from it directly — no prompt-pasting. Read-only:
  tools return DLS data, decisions, and check results; the client writes the code.

  Run:      node mcp/server.mjs        (a client launches it over stdio)
  Inspect:  npm --prefix mcp run inspect
*/
import { McpServer } from '@modelcontextprotocol/sdk/server/mcp.js'
import { StdioServerTransport } from '@modelcontextprotocol/sdk/server/stdio.js'
import { z } from 'zod'
import * as dls from './lib/dls.mjs'
...
server.tool(
  'get_component',
  'Get a 2one component by name (e.g. "button", "dialog", "app-bar").',
  { name: z.string() },
...
// ---- resources (read-only reference the client can pull) -------------------
